...
resource "powerscale_quota" "research_quota" {
  ...
  thresholds = {
    hard     = each.value.quota_gb * 1073741824
    soft     = floor(each.value.quota_gb * 1073741824 * 0.90)
    advisory = floor(each.value.quota_gb * 1073741824 * 0.80)
  }
...
resource "powerscale_snapshot_schedule" "research_snapshots" {
  ...
  retention_time = var.snapshot_retention_days * 86400
